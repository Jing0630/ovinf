...
#ifndef OVINF_HUMANOID_RUN_H
#define OVINF_HUMANOID_RUN_H
...
class HumanoidRunPolicy : public BasePolicy<float> {
 ...
 public:
  HumanoidRunPolicy() = delete;
  ~HumanoidRunPolicy();

  HumanoidRunPolicy(const YAML::Node &config): BasePolicy(config) {
  // Read config
  ...
  last_action_ = VectorT(14).setZero();
  latest_target_ = VectorT(14).setZero();
  ...
  worker_thread_ = std::thread(&HumanoidRunPolicy::WorkerThread, this);
}
...
HumanoidRunPolicy::~HumanoidRunPolicy() {
  exiting_.store(true);
  if (worker_thread_.joinable()) {
    worker_thread_.join();
  }
}
bool HumanoidRunPolicy::WarmUp(RobotObservation<float> const &obs_pack) {
...
bool HumanoidRunPolicy::InferUnsync(RobotObservation<float> const &obs_pack) {
  ...
  obs.segment(19, 14) = obs_pack.joint_vel * obs_scale_dof_vel_;
  obs.segment(33, 14) = last_action_;
  obs.segment(47, 3) = obs_pack.ang_vel * obs_scale_ang_vel_;
  obs.segment(50, 3) = obs_pack.proj_gravity * obs_scale_proj_gravity_;
...
std::optional<HumanoidRunPolicy::VectorT> HumanoidRunPolicy::GetResult(
...
void HumanoidRunPolicy::PrintInfo() {
...
void HumanoidRunPolicy::WorkerThread() {
  if (realtime_) {
...
void HumanoidRunPolicy::CreateLog(YAML::Node const &config) {
...
void HumanoidRunPolicy::WriteLog(RobotObservation<float> const &obs_pack) {
